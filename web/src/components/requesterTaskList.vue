      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{total(scope.row.assignmentReward,scope.row.partTimes,scope.row.assignTimes)}}</span>
          </template>
        </el-table-column>
        <el-table-column
          width="180">
    ...
    export default {
      ...
      methods:{
        total:function (assignmentReward,partTimes,assignTimes) {
          let res= parseFloat(Number(assignmentReward)*Number(partTimes)*Number(assignTimes)).toFixed(3);
          return res;
        },
        getConfiguration:function(){
          let userId=sessionStorage.getItem("userID");
          //console.log('hi')
          this.$api.get('/requester/'+userId+'/ongoing',null,r=>{
            console.log(r.data);
            this.tasks=r.data;
          })
        },
        ...
        openTask:function (taskId) {
          this.$router.push('/taskPartition/'+taskId);
        }
      }
    }
</script>
